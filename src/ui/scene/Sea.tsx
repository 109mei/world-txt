...
export function Sea({ v }: { v: SceneView }) {
  ...
  const seaFill = fresh > 0.3 ? '#1f2a3a' : '#141820';
  // 絵が二つ並んでも混ざらないよう、海の色ごとに別の名前にする
  const seaKind = fresh > 0.3 ? 'fresh' : 'salt';
  if (gone > 0.5) {
    return (
      <Motif v={v} id="noSea">
        <path d={`M${SHORE - 4} ${GROUND} L${W} ${HORIZON + 2} L${W} ${H} L${SHORE + 6} ${H} Z`} fill="#17150f" stroke={strokeOf(v, 'noSea', SILVER_DIM)} strokeWidth={0.8} />
    ...
    <g>
        ...
        <Motif v={v} id="seaCity">
          {[312, 342, 368].map((x, i) => (
            <g key={x} transform={`translate(${x} ${H - 8 - (i % 2) * 6})`}>
              <path d="M-9 0 A9 9 0 0 1 9 0 Z" fill="rgba(166,205,255,0.08)" stroke={strokeOf(v, 'seaCity')} strokeWidth={0.8} />
        ...
        <Motif v={v} id="seaFields">
          {[0, 1].map((k) => (
            <g key={k} transform={`translate(${308 + k * 36} ${level + 16 + k * 10})`}>
              <rect x={-12} y={-1} width={24} height={2.4} fill="#1a1d23" stroke={strokeOf(v, 'seaFields')} strokeWidth={0.6} />
      ...
      {land > 0 && (
        <Motif v={v} id="moreLand">
          <path d={`M318 ${level + 18} Q 340 ${level + 4 - 6 * land} 372 ${level + 16} Z`} fill="#0f1116" stroke={strokeOf(v, 'moreLand')} strokeWidth={0.8} />
      ...
      {Array.from({ length: ice > 0.6 ? 0 : ships }, (_, i) => (
        <g key={i} transform={`translate(${shoreTop + 26 + i * 30} ${level + 9 + i * 12})`}>
          <g className="sc-sail" style={{ ...dur(16 + i * 5), ...delay(i * 4) }}>
            <path d="M-8 0 L8 0 L5 3.5 L-5 3.5 Z" fill={DARK} stroke={SILVER} strokeWidth={0.7} />
            <path d="M0 0 L0 -9 L6 -2 Z" fill="#1c1d24" stroke={SILVER} strokeWidth={0.6} />
